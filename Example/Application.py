...
def index(path_info):
    ...
    try:
        print('运行index.......')
        conn = connect(host='localhost', port=3306, database='stock_db', user='root', password='201011621111', charset='utf8')
        cursor = conn.cursor()
...
def upadet_note_info(path_info):
    ...
    new_note_info = unquote(new_note_info)  # 对URL中可能存在的%编码进行URL解码
