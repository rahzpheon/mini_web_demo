...
class WSGIServer(object):
    ...
    def handle_client(self, client_socket):
        ...
        # 更新:为实现伪静态,将静态请求引入动态功能
        # 下面真正静态的处理暂时作废(运行不到)
        if not file_name.endswith('.html'):

            if not os.path.exists(self.documents_root + file_name):

                print('Your request:', file_name)
        ...
        # 伪静态时调用这里
            ...
            response = self.headers + response_body
